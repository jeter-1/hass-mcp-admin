...
- For Home Assistant behavior, delegated `ha-mcp` contracts, Home Assistant
  Cloud client behavior, add-on lifecycle, or host-platform claims, follow
...
- Compatibility fixtures for previously shipped persisted formats must be
  generated by the exact shipped code or captured from records written by that
  code. They must not be reconstructed manually from assumptions about the
  historical schema.
- Committed historical fixtures must include provenance identifying the exact
  source commit and generation process. Review must independently verify that
  the historical writer produced the claimed shape.
- Raw production records should not be committed by default. Sanitization must
  occur before historical code computes hashes, and hash-bearing serialized
  records must not be edited manually after generation.
- Fixture provenance is a review boundary. Beta 6 prohibited-plan
  compatibility may be removed only through an explicit storage-format
  decision after supported deployments have been scanned or migrated and no
  qualifying contract-v2 records remain. A retention interval alone is not a
  removal criterion.
