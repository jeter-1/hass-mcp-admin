...
Engineering **2.2.3** enforces received Host and Origin before MCP processing.
It supplements secret-path authentication with exact, bounded header policy.
Issue #62 remains open pending appropriate installed-path acceptance.
...
Read the [2.2.3 release notes](docs/V2_2_3_RELEASE_NOTES.md) and
[2.2.3 acceptance contract](docs/V2_2_3_ACCEPTANCE.md). A source version does
...
before an authorized 2.2.3 deployment. MCP aliases beyond the strict loopback
...
5. Verify the final installed artifact, reconnect the client and complete the
   bounded [acceptance checks](docs/V2_2_3_ACCEPTANCE.md).

The image repository remains
`ghcr.io/jeter-1/hass-mcp-engineering-beta`, supporting `linux/amd64`,
`linux/arm64` for 2.2.3. Published 2.2.0 retains its historical
...
ports, ingress and technical Beta identities are preserved. The two header-policy
options add strict admission defaults; reconcile them before updating.
...
referencing existing compiled contracts. Installing 2.2.3 does not configure
...
original release and do not replace the current 2.2.3 contract.
