...
Engineering **2.4.0-beta.3** is the materialized candidate for
[dashboard planning capacity and safe diagnostics](docs/F3_DASHBOARD_WRITE_CONTRACT.md#semantic-capacity-and-safe-diagnostics).
It raises the semantic patch ceiling from 16 to 256 leaves while preserving the
other bounds, approval binding, one-dispatch execution and exact readback.
Oversized patches report their actual count and limit without configuration content.
...
Read the [2.4.0-beta.3 release notes](docs/V2_4_0_BETA3_RELEASE_NOTES.md) and
[2.4.0-beta.3 acceptance contract](docs/V2_4_0_BETA3_ACCEPTANCE.md). A source version does
...
before an authorized 2.4.0-beta.3 deployment. MCP aliases beyond the strict loopback
...
5. Verify the final installed artifact, reconnect the client and complete the
   bounded [acceptance checks](docs/V2_4_0_BETA3_ACCEPTANCE.md).

The image repository remains
`ghcr.io/jeter-1/hass-mcp-engineering-beta`, supporting `linux/amd64`,
`linux/arm64` for 2.4.0-beta.3. Published 2.2.0 retains its historical
...
signed typed references remain unavailable as intended. Installing 2.4.0-beta.3 neither
...
original release and do not replace the current 2.4.0-beta.3 contract.
