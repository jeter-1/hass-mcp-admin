...
def core_entry(version="2026.9.2", *, typed_operations=False):
    return {
        ...
        )} for p in CORE_CAPABILITY_PROFILES + (CORE_TYPED_OPERATION_PROFILES if typed_operations else ())],
    }
...
class ProjectedCoreSource:
    ...
    def __init__(self, registry, version="2026.9.2", *, typed_operations=False):
        self.registry, self.version = registry, version
        self.typed_operations = typed_operations
    ...
    async def capture_core_snapshot(self):
        ...
        evidence = capability_evidence_for_probes(
            include_typed_operations=self.typed_operations,
