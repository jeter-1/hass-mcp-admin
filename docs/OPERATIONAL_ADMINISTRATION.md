...
- `self_addon_identity_unavailable` means Supervisor self metadata was
  ...
  fixed 512 KiB ceiling, reads every transport fragment to EOF, retains only
  validated identity fields, and exposes no raw Supervisor content.
  Approval-notification health may refine an identity
