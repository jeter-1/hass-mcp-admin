...
3. retrieves the complete paginated upstream catalog;
...
It is the complete classification table for the 78 tools advertised by reviewed
`ha-mcp` 7.14.1 source tag `v7.14.1`, commit
`255acec1affa6528004a122eb83e30aee9c77713`. The loader requires all entries to
be sorted, unique, bounded, and from that exact source revision. An advertised
tool is not callable merely because it exists upstream.
...
- exact source/catalog evidence references.
...
|---|---:|---|
| `automatic_read` | 27 | Exact schema match is dynamically registered. |
| `mixed_or_requires_wrapper` | 13 | Unavailable generically; an explicit constrained wrapper is required. |
...
- `ha_eval_template`, `ha_get_automation_traces`, `ha_get_blueprint`,
  ...
  `ha_get_hacs_info`, `ha_get_history`, `ha_get_logs`,
...
Engineering publishes its own fixed read-only annotations. Returned JSON-RPC-
...
- Mixed, write, action, prohibited, and unsupported entries never reach the
  generic provider.
- Discovery/admission failure withholds only dynamic reads; existing
  Engineering-native tools continue to start.

Health and capability output distinguish advertised, reviewed, automatic-read,
dynamically exposed, collision, blocked, missing-reviewed-read,
schema-mismatch, and unreviewed counts.
It also reports the observed version/catalog fingerprint and makes explicit
that advertised does not mean callable.

## Usability effect

Normal tasks such as finding entities, inspecting devices, reading automation
configuration, checking state/history, reviewing traces/logs, listing services,
