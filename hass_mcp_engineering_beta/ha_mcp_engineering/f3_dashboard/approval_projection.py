...
def build_dashboard_approval_projection(
    ...
    *,
    ...
    """Project every declared before/after value without unrelated content."""
        ...
        operations.append(
            {
                ...
                "path": effect.path,
                "previous": _snapshot(
                    present=effect.previous_present, value=effect.previous_value
                ),
                "proposed": _snapshot(
                    present=effect.proposed_present, value=effect.proposed_value
                ),
            }
        )
...
def validate_dashboard_approval_projection(
    ...
    *,
    ...
    for row in operations:
        ...
        try:
            tokens = parse_pointer(row.get("path"), operation=kind)
        except ValueError as exc:
            raise _failure(
    ...
    sanitation = sanitize_untrusted_data(projection, known_secrets=known_secrets)
